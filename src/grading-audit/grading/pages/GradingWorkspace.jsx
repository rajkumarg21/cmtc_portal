...
/* Grading */
import InternalGrading from "./InternalGrading";
// import ExternalGrading from "./ExternalGrading";
import ExternalGradingSelf from "./ExternalGradingSelf";
import ExternalGradingAssigned from "./ExternalGradingAssigned";
...
const GradingWorkspace = () => {
  const {
    ...
    viewType,
    setViewType,
  } = useAssessment(CycleCategory.GRADING);
  ...
  const {
    ...
    error: cycleError,
  } = useCycles(CycleCategory.GRADING, cycleType);


  const selectedCycleId = selectedCycle?.id || selectedCycle?.cycleId || "";
  const { user } = useAuth();
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        ...
        {cycleError && (
          <Typography color="error" mt={2}>
            Failed to load assessment cycles.
          </Typography>
        )}
      </Paper>
