...
import{
 deleteActionPlanContent,
 getAllActionPlan,
 getAllActionPlansList,
 getActionPlanById,
 updateActionPlan,
 createActionPlan,
approveActionPlan,
 rejectActionPlan,  } from  '../../../services/actionPlanService';
...
const ActionPlanManagementPage = () => {
  const { id } = useParams();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const data = await getAllActionPlansList(page, pageSize, "id", "DESC");
          setEvents(data.content || []);
          setTotalRows(data.totalElements || 0);
 
      if (isEditing) {
        const pageData = await getActionPlanById(id);
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
       await  updateActionPlan(id, payload, imageFile, pdfFile, videoFile);
         setMessage('Event updated successfully and sent for approval!');
      } else {
        await createActionPlan(payload, imageFile, pdfFile, videoFile);
  ...
  const handleDeleteClick = (row) => {
    ...
    const handleConfirmDelete = async () => {
        setLoading(true);
        try {
          await deleteActionPlanContent(itemToDelete.id);
    ...
    const handleApproveClick = (row) => {
      ...
      const handleConfirmApprove = async () => {
        setLoading(true);
        try {
          await approveActionPlan(itemToApprove.id);
      ...
      const handleConfirmReject = async () => {
        setLoading(true);
        try {
          await rejectActionPlan(itemToReject.id);
 ...
 return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      <Typography variant="h4" align="center" gutterBottom>
        {isEditing ? "Edit Action Plan" : "Annual Action Plan"}
      </Typography>

      {/* ======== FORM SECTION ======== */}
      <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
        <Typography variant="h5" gutterBottom align="center">
          {isEditing ? "Edit Action Plan Content" : "Add Action Plan Content"}
        </Typography>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6} sx={{ textAlign: "center", mt: 2 }}>
              <Button type="submit" variant="contained">
                {isEditing ? "Update Action Plan " : "Submit Action Plan"}
              </Button>
            </Grid>
          </Grid>
        </form>
      ...
      <Paper sx={{ p: 3 }}>
        <Typography variant="h6" align="center" gutterBottom>
                         All Annual Action Plan
              </Typography>
       {loading ? (
                  <Box sx={{ display: 'flex', justifyContent: 'center', my: 4 }}><CircularProgress /></Box>
                ) : events.length === 0 ? (
                  <Typography align="center">No Annual Action Plan found.</Typography>
                ) : (
                  <DataGrid
                    autoHeight
                    rows={events}
...
export default ActionPlanManagementPage;