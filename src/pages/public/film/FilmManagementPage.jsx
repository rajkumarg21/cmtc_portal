...
const FilmManagementPage = () => {
  ...
  // Fetch Livelihood Activities data
  const fetchData = useCallback(async () => {
    ...
    try {
      const data = await getAllFilmsList(page, pageSize, "id", "DESC");
       setFilms(data.content || []);
       setTotalRows(data.totalElements || 0);

      if (isEditing) {
        const pageData = await getFilmPageById(id);
        setFormData({
  ...
  }, [page, pageSize,id, isEditing, hasRole]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await updateFilmPage(id, payload, imageFile, pdfFile, videoFile);
        setMessage("Livelihood Activities updated successfully and sent for approval!");
      } else {
        await createFilmPage(payload, imageFile, pdfFile, videoFile);
        setMessage("Livelihood Activities added successfully and sent for approval!");
      }
      ...
      setTimeout(() => navigate("/cms/films"), 500);
  ...
  const handleConfirmDelete = async () => {
      setLoading(true);
      try {
        await deleteFilm(itemToDelete.id);
        toast.success('Livelihood Activities deleted successfully!');
        await fetchData();
      } catch (err) {
  ...
  const handleApproveClick = (row) => {
    ...
    const handleConfirmApprove = async () => {
      setLoading(true);
      try {
        await approveFilmPage(itemToApprove.id);
        toast.success('Livelihood Activities approved!');
        await fetchData();
      } catch (err) {
    ...
    const handleConfirmReject = async () => {
      setLoading(true);
      try {
        await rejectFilmPage(itemToReject.id);
        toast.success('Livelihood Activities Reject!');
        await fetchData();
      } catch (err) {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          <IconButton
            color="primary"
            component={Link}
            to={`/cms/film/edit/${params.row.id}`}
            size="small"
          >
            <EditIcon />
          </IconButton>
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      <Typography variant="h4" align="center" gutterBottom>
        {isEditing ? "Edit Livelihood Activities" : "Livelihood Activities Content Management"}
      </Typography>

     
      <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
        <Typography variant="h5" gutterBottom align="center">
          {isEditing ? "Edit Livelihood Activities Content" : "Add Livelihood Activities Content"}
        </Typography>
          ...
          <Grid container spacing={2}>
                         ...
                         const file = e.target.files[0];
                          if (file) {
                           if (file.size > 100 * 1024 * 1024) {
                              setPdfFile(null);
                              setError("Image file size should not exceed 100MB.");
                             e.target.value = null;
                            return;
                           }
                           setError("");
            ...
            <Grid item xs={12} md={6} sx={{ textAlign: "center", mt: 2 }}>
              <Button type="submit" variant="contained">
                {isEditing ? "Update Livelihood Activities" : "Submit Livelihood Activities"}
              </Button>
            </Grid>
          </Grid>
        </form>
      ...
      <Paper sx={{ p: 3 }}>
       <Typography variant="h6" align="center" gutterBottom>
         ...
         {loading ? (
                  ...
                  <Typography align="center">No Livelihood Activities found.</Typography>
                ) : (
                  <DataGrid
                    autoHeight
                    rows={films}
              ...
              <ConfirmationDialog
                ...
                message={`Delete Livelihood Activities"${itemToDelete?.titleEnglish}"?`}
              />
              ...
              <ConfirmationDialog
                ...
                message={`Approve Livelihood Activities "${itemToApprove?.titleEnglish}"?`}
              />
              <ConfirmationDialog
                ...
                message={`Reject Livelihood Activities "${itemToReject?.titleEnglish}"?`}
              />
            </Container>
          );
};
