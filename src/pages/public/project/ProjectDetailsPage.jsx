...
import { getProjectById , getLatestPublishedProject} from "../../../services/projectService";
...
const ProjectDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchOther = async () => {
      ...
      try {
        ...
        const items = await getLatestPublishedProject(candidateCount);
        if (!mounted) return;
  ...
  return (
    <Box>
       <PageHeader title={t('sections.project_section')} />
      ...
      <Box sx={{ padding: 4 }}>
        <Grid container spacing={4} sx={{ flexWrap: "nowrap" }}>
              ...
              <Box sx={{ position: "relative" }}>
                {article.imageUrl ? (
                  <CardMedia
                    ...
                    height="420"
                    controls
                    src={`${import.meta.env.VITE_BASE_URL || ""}${article.imageUrl}`}
                    sx={{ objectFit: "cover" }}
                  />
                ) : (
                  <CardMedia
                    component="img"
                    height="420"
                    image="https://placehold.co/800x400/F0F7FF/60A5FA?text=Project+Detail"
                  />
                )}
          ...
          >
            ...
            >
              {isHindi ? "अन्य महत्वपूर्ण " : "Other Important Projects"}
            </Typography>
            ...
            {otherLoading ? (
              ...
              <Typography variant="body2" color="text.secondary">
                {isHindi ? "कोई अन्य  सेवा उपलब्ध नहीं है।" : "No other services available."}
              </Typography>
            ) : (
              <List disablePadding>
                {otherNews.map((item) => {
                  ...
                  return (
                    <ListItemButton
                      key={item.id}
                      component={Link}
                      // Navigate to the project detail page path
