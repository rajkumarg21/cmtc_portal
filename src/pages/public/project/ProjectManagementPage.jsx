...
approveProject,
 rejectProject,
 deleteProjectContent,  } from  '../../../services/projectService';
...
const ProjectManagementPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const data = await getAllProjectsList(page, pageSize, "id", "DESC");
        setProjects(data.content || []);
        setTotalRows(data.totalElements || 0);

      if (isEditing) {
        const pageData = await getProjectById(id);
        setFormData({
      ...
      setProjects([]); // Ensure state is reset on error
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
       await  updateProject(id, payload, imageFile, pdfFile, videoFile);
         setMessage('Act and Rules updated successfully and sent for approval!');
      } else {
        await createProject(payload, imageFile, pdfFile, videoFile);
         setMessage('Act and Rules added successfully and sent for approval!');
      }
       ...
       setTimeout(() => { navigate('/cms/projects');}, 500); 
   ...
   const handleDeleteClick = (row) => {
      ...
      const handleConfirmDelete = async () => {
          setLoading(true);
          try {
            await deleteProjectContent(itemToDelete.id);
            toast.success('Act and Rules deleted successfully!');
            await fetchData();
          } catch (err) {
      ...
      const handleApproveClick = (row) => {
        ...
        const handleConfirmApprove = async () => {
          setLoading(true);
          try {
            await approveProject(itemToApprove.id);
            toast.success('Act and Rules approved!');
            await fetchData();
          } catch (err) {
        ...
        const handleConfirmReject = async () => {
          setLoading(true);
          try {
            await rejectProject(itemToReject.id);
            toast.success('Act and Rules Reject!');
            await fetchData();
          } catch (err) {
          ...
          renderCell: (params) => (
            <Box sx={{ display: 'flex', gap: 1 }}>
              <IconButton
                color="primary"
                component={Link}
                to={`/cms/Project/edit/${params.row.id}`}
                size="small"
              >
                <EditIcon />
              </IconButton>
 ...
 return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      <Typography variant="h4" align="center" gutterBottom>
        {isEditing ? "Edit Act and Rules" : "Act and Rules Management"}
      </Typography>

      {/* ======== FORM SECTION ======== */}
      <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
        <Typography variant="h5" gutterBottom align="center">
          {isEditing ? "Edit Act and Rules Content" : "Add Act and Rules Content"}
        </Typography>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6} sx={{ textAlign: "center", mt: 2 }}>
              <Button type="submit" variant="contained">
                {isEditing ? "Update Act and Rules" : "Submit Act and Rules"}
              </Button>
            </Grid>
          </Grid>
        </form>
     ...
     {/* ======== TABLE SECTION ======== */}
      <Paper sx={{ p: 3 }}>
       ...
       {loading ? (
                  ...
                  <Typography align="center">No Act and Rules found.</Typography>
                ) : (
                  <DataGrid
                    autoHeight
                    rows={projects}
          ...
          {/* ======== Dialog ======== */}
              ...
              <ConfirmationDialog
                ...
                message={`Delete  Act and Rules "${itemToDelete?.titleEnglish}"?`}
              />
              ...
              <ConfirmationDialog
                ...
                message={`Approve Act and Rules  "${itemToApprove?.titleEnglish}"?`}
              />
              <ConfirmationDialog
                ...
                message={`Reject Act and Rules "${itemToReject?.titleEnglish}"?`}
              />
